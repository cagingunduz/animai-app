            ...
            {chars.length > 0 && (
              <div className="flex-shrink-0 border-t border-[rgba(255,255,255,0.1)] px-5 md:px-7 py-4">
                ...
                <div className="flex gap-2.5 overflow-x-auto pb-1">
                  {chars.map(c => (
                    <div key={c.id} className="relative group w-[100px] flex-shrink-0">   <button onClick={() => openEditChar(c)} className="w-full border border-[rgba(255,255,255,0.1)] rounded-[10px] overflow-hidden hover:border-[rgba(255,255,255,0.25)] transition-all bg-[#0f0f0f] text-left">     <div className="h-[68px] bg-[#131313] flex items-center justify-center overflow-hidden">       {c.imageUrl ? <img src={c.imageUrl} alt={c.name} className="w-full h-full object-cover object-top" /> : <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="rgba(255,255,255,0.1)" strokeWidth="1.5"><circle cx="12" cy="8" r="4"/><path d="M6 21v-2a4 4 0 014-4h4a4 4 0 014 4v2"/></svg>}     </div>     <div className="px-2 py-1.5 text-[10px] text-center text-[rgba(255,255,255,0.5)] truncate">{c.name}</div>   </button>   <button     onClick={() => setChars(prev => prev.filter(x => x.id !== c.id))}     className="absolute top-1 right-1 w-5 h-5 rounded-full bg-black/80 border border-[rgba(255,255,255,0.1)] items-center justify-center text-[rgba(255,255,255,0.5)] hover:text-white hover:border-[rgba(255,255,255,0.3)] transition-all text-[10px] hidden group-hover:flex">×</button> </div>
